...
// node_modules/@martianwallet/aptos-wallet-adapter/dist/index.mjs
var MartianWalletName = "Martian";
var MartianWallet = class {
  constructor() {
  ...
  async connect() {
    ...
    try {
      ...
      if (!addressInfo)
        throw `${MartianWalletName} Address Info Error`;
      return addressInfo;
    } catch (error) {
      throw error;
    }
  ...
  async account() {
    ...
    if (!response)
      throw `${MartianWalletName} Account Error`;
    return response;
  }
  async disconnect() {
    var _a;
  ...
  async signAndSubmitTransaction(transaction, options) {
    ...
    try {
      ...
      const tx = await ((_b = this.provider) == null ? void 0 : _b.generateTransaction(
        signer.address,
        transaction,
        { ...options, max_gas_amount: (_a = options == null ? void 0 : options.max_gas_amount) == null ? void 0 : _a.toString() }
      ));
      if (!tx)
        throw new Error(
          "Cannot generate transaction"
        );
      const response = await ((_c = this.provider) == null ? void 0 : _c.signAndSubmitTransaction(tx));
      if (!response) {
  ...
  async signAndSubmitBCSTransaction(transaction, options) {
    var _a;
    try {
      const serializer = new bcs_exports.Serializer();
      transaction.serialize(serializer);
      const response = await ((_a = this.provider) == null ? void 0 : _a.signAndSubmitBCSTransaction(
  ...
  async signTransaction(transaction, options) {
    ...
    try {
      const signer = await this.account();
      const tx = await ((_a = this.provider) == null ? void 0 : _a.generateTransaction(
        signer.address,
        transaction,
        options
      ));
      ...
      const response = await ((_b = this.provider) == null ? void 0 : _b.signTransaction(tx));
      if (!response) {
        throw new Error("No response");
      }
      return response;
  ...
  async network() {
    ...
    try {
      ...
      if (!response)
        throw `${MartianWalletName} Network Error`;
      return {
        name: response,
...
//# sourceMappingURL=@martianwallet_aptos-wallet-adapter.js.map
